...
,diag(b_sim.dic),diag(b_sim.we),diag(b_short),diag(b_NB),diag(b_RF),diag(b_gbm),diag(b_svm),diag(b_lda),diag(b_glmnet),diag(b_ens),diag(b_ens1),diag(b_ens2),diag(b_ens3),diag(b_ens4),diag(b_ens5),diag(b_ens6),diag(b_ens7),diag(b_ens8),
...
colnames(com)=c("Degree","cregiyg","vregiyg_shared","Betwee","clos","klein","sim.jacard","sim.dic","sim.we","short","NaiveBayes","RF","GBM","SVM","LDA","Glmnet","RF+SVM+LDA","RF+LDA","RF+NaiveBayes+LDA","RF+SVM+Glmnet","RF+SVM+LDA+NaiveBayes","RF+SVM+LDA+NaiveBayes+Glmnet","RF+LDA+Glmnet","RF+Glmnet","LDA+Glmnet",
"ens1","ens2","ens3","ens4","ens5","ens6","ens7","ens8","ens9","ens10")


write.table(com,file="~/prioritization/Leave-one-out/New/Without_GO/Results/GO1.txt",sep="\t",col.names=T,row.names=F)