...
for(my $i=1;$i<148;$i++){
  my $jobfile="NS".$i."_Total_Leave.r";
  open(OUT,">$jobfile");
...
load(\"~/prioritization/Leave-one-out/New/GO/GO.RData\")  # GO analysis
load(\"~/prioritization/Leave-one-out/New/GO/GO_separated.RData\")
load(\"~/prioritization/Leave-one-out/New/Neighbor_similarity/NS.RData\")
...
# Reading each left-out dataset
  ...
  n=as.data.frame(ns[i])
  s=s[,-c(2,4,6,11,13,15,20,22,24)]
  colnames(s)=c(\"PTC\",\"PFC\",\"P5C\",\"JQC\",\"JNC\",\"JC\",\"PTF\",\"PFF\",\"P5F\",\"JQF\",\"JNF\",\"JF\",\"PTP\",\"PFP\",\"P5P\",\"JQP\",\"JNP\",\"JP\")
  a=cbind(c(1:dim(a)[1]),a,b[,-c(2,4,6)],s,n,y) # Combination of two data sets except odd ratio values
  a[a[,11]==\"Inf\",11]=1000
  a1=a[-cregiyg[i],]
  ...
  # Logistic Regression 
  data=data.frame(Y=a1\$y,alldeg=a1[,2],decregiyg=a1[,3],cregiyg_sh=a1[,4],bet=a1[,5],clos=a1[,6],klein=a1[,7],jaccard=a1[,8],sim.dic=a1[,9],sim.we=a1[,10],short=a1[,11],pvalue_TopTen=a1[,12],pvalue_first=a1[,13],pvalue_5=a1[,14],Jaccard_Q=a1[,15],Jaccard_N=a1[,16],Jaccard=a1[,17],JQP=a1[,33])
  model=glm(Y~alldeg+decregiyg+cregiyg_sh+bet+clos+klein+jaccard+sim.dic+sim.we+short+pvalue_TopTen+pvalue_first+pvalue_5+Jaccard_Q+Jaccard_N+Jaccard+JQP,data=data,family=\"binomial\")
  newglm=a[cregiyg[i],c(2:17,33)]
...
write.table(com,file=\"~/prioritization/Leave-one-out/New/Neighbor_similarity/RE".$i.".txt\",sep=\"\\t\",col.names=T,row.names=F)";
   close OUT;
}
