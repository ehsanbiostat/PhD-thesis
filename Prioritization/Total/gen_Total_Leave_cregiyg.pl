...
diag(b_NB),diag(b_RF),diag(b_gbm),diag(b_svm),diag(b_lda),diag(b_glmnet),diag(b_ens),diag(b_ens1),diag(b_ens2),diag(b_ens3),diag(b_ens4),diag(b_ens5),diag(b_ens6),diag(b_ens7),diag(b_ens8),
...
write.table(com,file=\"~/prioritization/Leave-one-out/New/GO/Results/GO_SVM".$i.".txt\",sep=\"\\t\",col.names=T,row.names=F)";
   close OUT;
}
