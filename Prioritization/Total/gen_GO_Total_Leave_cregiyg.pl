...
for(my $i=1;$i<147;$i++){
  my $jobfile="GO".$i."_Total_Leave.r";
  open(OUT,">$jobfile");
  print OUT  "# Reading data
load(\"~/prioritization/Leave-one-out/Total/Source/DATA(1).RData\")  # Whole Network data sets
...
# Reading each left-out dataset
  ...
  # Logistic Regression 
  data=data.frame(Y=a1\$y,Degree=a1[,2],cregiyg=a1[,3],shared_cregiyg=a1[,4],Betwee=a1[,5],clos=a1[,6]
  ,klein=a1[,7],sim.jacard=a1[,8],sim.dic=a1[,9],sim.we=a1[,10],short=a1[,11],pvalue_TopTen=a1[,12],pvalue_first=a1[,13],Jaccard_Q=a1[,14],Jaccard_N=a1[,15],Jaccard=a1[,16])
  model=glm(Y~Degree+cregiyg+shared_cregiyg+Betwee+clos+klein+sim.jacard+sim.dic+sim.we+short
  +pvalue_TopTen+pvalue_first+Jaccard_Q+Jaccard_N+Jaccard,data=data,family=\"binomial\")
  newglm=a[cregiyg[i],c(2:16)]
  ...
  data=data.frame(Y=a1\$y,Degree=a1[,2],cregiyg=a1[,3],shared_cregiyg=a1[,4],Betwee=a1[,5],clos=a1[,6]
  ,klein=a1[,7],sim.jacard=a1[,8],sim.dic=a1[,9],sim.we=a1[,10],short=a1[,11],Jaccard_Q=a1[,14])
  model=glm(Y~Degree+cregiyg+shared_cregiyg+Betwee+clos+klein+sim.jacard+sim.dic+sim.we+short
  +Jaccard_Q,data=data,family=\"binomial\")
  newglm=a[cregiyg[i],c(2:11,14)]
  p=predict(model,newglm,type=c(\"response\"))
  fit=predict(model,type=c(\"response\"))
  a=cbind(a,c(fit[1:c(cregiyg[i]-1)],p,fit[cregiyg[i]:length(fit)]))
  ...
  a_Jaccard_Q=a[order(-a[,14]),]
  a_Jaccard_N=a[order(-a[,15]),]
  a_Jaccard=a[order(-a[,16]),] 
  a_NB=a[order(-a[,18]),]
  a_RF=a[order(-a[,19]),]
  a_gbm=a[order(-a[,20]),]
  a_log=a[order(-a[,21]),]
...
write.table(com,file=\"~/prioritization/Leave-one-out/Total/Results/results_Ens_GO".$i.".RData\",sep=\"\\t\",col.names=T,row.names=F)";
   close OUT;
}
