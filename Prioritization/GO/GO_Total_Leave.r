# Reading data
load("~/prioritization/Leave-one-out/New/Total_topology/Leave-one-out.RData")  # Whole Network data sets
...
# Reading each left-out dataset
  ...
  a=as.data.frame(d[i])
  b=as.data.frame(g[i])
  a=cbind(c(1:dim(a)[1]),a,b[,-c(2,4,6)],y) # Combination of two data sets except odd ratio values
  a[a[,11]=="Inf",11]=1000
  a1=a[-cregiyg[i],]
  ...
  ,klein=a1[,7],jaccard=a1[,8],sim.dic=a1[,9],sim.we=a1[,10],short=a1[,11],pvalue_TopTen=a1[,12],pvalue_first=a1[,13],pvalue_5=a1[,14],Jaccard_Q=a1[,15],Jaccard_N=a1[,16],Jaccard=a1[,17])
  model=glm(Y~alldeg+decregiyg+cregiyg_sh+bet+clos+klein+jaccard+sim.dic+sim.we+short
  +pvalue_TopTen+pvalue_first+pvalue_5+Jaccard_Q+Jaccard_N+Jaccard,data=data,family="binomial")
  newglm=a[cregiyg[i],c(2:17)]
  ...
  data=data.frame(Y=a1$y,alldeg=a1[,2],decregiyg=a1[,3],cregiyg_sh=a1[,4],bet=a1[,5],clos=a1[,6]
  ,klein=a1[,7],jaccard=a1[,8],sim.dic=a1[,9],sim.we=a1[,10],short=a1[,11],Jaccard_Q=a1[,15])
  model=glm(Y~alldeg+decregiyg+cregiyg_sh+bet+clos+klein+jaccard+sim.dic+sim.we+short
  +Jaccard_Q,data=data,family="binomial")
  newglm=a[cregiyg[i],c(2:11,15)]
  p=predict(model,newglm,type=c("response"))
  fit=predict(model,type=c("response"))
  a=cbind(a,c(fit[1:c(cregiyg[i]-1)],p,fit[cregiyg[i]:length(fit)]))
...
write.table(com,file="~/prioritization/Leave-one-out/Total/Results/results_Ens_GO1.RData",sep="\t",col.names=T,row.names=F)