...
for(my $i=1;$i<148;$i++){
  my $jobfile="GO".$i."_Mutant_separated.r";
  open(OUT,">$jobfile");
  print OUT  "load(\"~/prioritization/Combined_Network/New/Leave_one_out/Mutant.RData\")
...
save(w, file=\"~/prioritization/Leave-one-out/New/Leave-one-out-network/GO/Mutant_GO_separated".$i.".RData\")";
   close OUT;
}
