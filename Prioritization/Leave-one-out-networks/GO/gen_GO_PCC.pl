...
for(my $i=1;$i<148;$i++){
  my $jobfile="GO".$i."_PCC.r";
  open(OUT,">$jobfile");
  print OUT  "go=read.table(\"~/prioritization/Combined_Network/New/GO_ID.txt\",sep=\"\\t\")
load(\"~/prioritization/Combined_Network/New/Leave_one_out/PCC.RData\")
iyg=read.table(\"~/prioritization/Combined_Network/New/Filtering_list/cregiyg.txt\") # IYG and REG
go=go[,-1]
iyg=as.vector(unlist(iyg[,2]))
GOiyg=subset(go,go[,2]%in%iyg[-".$i."])[,1] # GO of IYG & REG
...
write.table(c, file=\"~/prioritization/Leave-one-out/New/Leave-one-out-network/GO/PCC_GO".$i.".txt\",col.names=T,row.names=F,sep=\"\\t\")";
   close OUT;
}
