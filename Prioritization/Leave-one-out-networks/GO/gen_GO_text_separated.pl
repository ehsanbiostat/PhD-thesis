...
for(my $i=1;$i<148;$i++){
  my $jobfile="GO".$i."_text_separated.r";
  open(OUT,">$jobfile");
  print OUT  "load(\"~/prioritization/Combined_Network/New/Leave_one_out/text.RData\")
...
save(w, file=\"~/prioritization/Leave-one-out/New/Leave-one-out-network/GO/text_GO_separated".$i.".RData\")";
   close OUT;
}
