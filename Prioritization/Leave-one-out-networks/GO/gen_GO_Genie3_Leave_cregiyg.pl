...
for(my $i=1;$i<148;$i++){
  my $jobfile="GO".$i."_Genie3_Leave.r";
  open(OUT,">$jobfile");
  print OUT  "load(\"~/prioritization/Leave-one-out/New/Leave-one-out-network/Genie3.RData\")  # Whole Network data sets
d=w
cregiyg=read.table(\"~/prioritization/Combined_Network/New/Filtering_list/cregiyg.txt\") # Known genes list (IYG & REG)
load(\"~/../../group/biocomp/users/ehsab/Leave-one-out/GO/Genie3_Leave-one-out.RData\")  # GO analysis
load(\"~/../../group/biocomp/users/ehsab/Leave-one-out/GO/Genie3_GO_separated.RData\")
...
write.table(com,file=\"~/prioritization/Leave-one-out/New/Leave-one-out-network/Results/Genie3/Genie3_".$i.".txt\",sep=\"\\t\",col.names=T,row.names=F)";
   close OUT;
}
