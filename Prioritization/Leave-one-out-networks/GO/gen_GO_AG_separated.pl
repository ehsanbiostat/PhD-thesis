...
for(my $i=81;$i<148;$i++){
  my $jobfile="GO".$i."_AG_separated.r";
  open(OUT,">$jobfile");
  print OUT  "load(\"~/prioritization/Combined_Network/New/Leave_one_out/AG.RData\")
...
save(w, file=\"~/prioritization/Leave-one-out/New/Leave-one-out-network/GO/AG_GO_separated".$i.".RData\")";
   close OUT;
}
