...
low.level.express = read.table(\"/group/biocomp/users/ehsab/Gene_duplication/Results/Expression_level/low.level.genes.txt\", header = F)

data = as.matrix(data)
## 18888 genes with 2933 conditions
...
## 6238 seed genes after filtering those low expressed genes
...
write.table(gene_scores, file = \"~/../../group/biocomp/users/ehsab/Gene_duplication/Results/SA/Gene_score/G_lowexpr".$i.".txt\", row.names = F, col.names = T)
write.table(cond_scores, file = \"~/../../group/biocomp/users/ehsab/Gene_duplication/Results/SA/Condition_score/C_lowexp".$i.".txt\", row.names = F, col.names = T)"
	;
	close OUT;
}
