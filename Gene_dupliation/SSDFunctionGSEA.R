...
SSDPollenRef = ref[SSDPollen, 2]
SSDLeafRef = ref[SSDPollenLeaf, 2]
SSDRootRef = ref[SSDRoot, 2]
SSDLeafRootRef = ref[SSDRootLeaf, 2]
...
GeneScoreRoot = GeneScore(RootRef, SSDRootRef, SSDRoot, Root)
GeneScoreLeafRoot = GeneScore(LeafRootRef, SSDLeafRootRef, SSDRootLeaf, LeafRoot)

GeneScorePollen = GeneScore(PollenRef, SSDPollenRef, SSDPollen, Pollen)
GeneScoreLeaf = GeneScore(LeafRef, SSDLeafRef, SSDPollenLeaf, Leaf)
...
write.table(rbind(GSEAPollen, GSEAPollenLeaf), file = "/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/Genes/SSD_GSEA_PollenLeaf_Top50_500.txt", row.names = F, quote = F , sep = "\t")
write.table(rbind(GSEARoot, GSEARootLeaf), file = "/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/Genes/SSD_GSEA_RootLeaf_Top50_500.txt", row.names = F, quote = F , sep = "\t")
...
write.table(ReorderMatrix(mydata), file =paste("/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/GO/GSEA/SSD_GSEA_RootLeaf",Type,".txt", sep = ""), quote = F, sep = "\t")
annotation = data.frame("Module" = ifelse(colnames(mydata) %in% SSDRoot, "Root", "Leaf"), "Noverlap" = ifelse(colnames(mydata) %in% SSDRoot & !(colnames(mydata) %in% SSDPollen), "Root",
	ifelse(colnames(mydata) %in% SSDRoot & colnames(mydata) %in% SSDPollen, "Root-Pollen",
		ifelse(colnames(mydata) %in% SSDRootLeaf & colnames(mydata) %in% SSDPollenLeaf, "LeafLeaf", "Leaf"))))
...
pheatmap(t(mydata), color = colorRampPalette(c("white", "black"))(10), clustering_distance_rows = "euclidean", clustering_distance_cols = "euclidean", clustering_method = "ward.D2",
	...
	filename = paste("/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/GO/GSEA/SSD_GSEA_RootLeaf",Type,".pdf", sep = ""))


mydata = Mydata(PollenLeaf, 0.1)
write.table(ReorderMatrix(mydata), file =paste("/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/GO/GSEA/SSD_GSEA_PollenLeaf",Type,".txt", sep = ""), quote = F, sep = "\t")
annotation = data.frame("Module" = ifelse(colnames(mydata) %in% SSDPollen, "Pollen", "Leaf"), "Noverlap" = ifelse(!(colnames(mydata) %in% SSDRoot) & colnames(mydata) %in% SSDPollen, "Pollen",
	ifelse(colnames(mydata) %in% SSDRoot & colnames(mydata) %in% SSDPollen, "Root-Pollen",
		ifelse(colnames(mydata) %in% SSDRootLeaf & colnames(mydata) %in% SSDPollenLeaf, "LeafLeaf", "Leaf"))))
rownames(annotation) = colnames(mydata)
pheatmap(t(mydata), color = colorRampPalette(c("white", "black"))(10), clustering_distance_rows = "euclidean", clustering_distance_cols = "euclidean", clustering_method = "ward.D2",
	...
	filename = paste("/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/GO/GSEA/SSD_GSEA_PollenLeaf",Type,".pdf", sep = ""))


mydata = Mydata(PollenRootLeafGO, 0.1)
write.table(ReorderMatrix(mydata), file =paste("/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/GO/GSEA/SSD_GSEA_PollenRootLeafGO",Type,".txt", sep = ""), quote = F, sep = "\t")
annotation = data.frame("Module" = factor(ifelse(colnames(mydata) %in% SSDPollen, "Pollen", ifelse(colnames(mydata) %in% SSDRoot, "Root", ifelse(colnames(mydata) %in% SSDRootLeaf, "Root-Leaf", "Pollen-Leaf")))))
rownames(annotation) = colnames(mydata)
pheatmap(t(mydata), color = colorRampPalette(c("white", "black"))(10), clustering_distance_rows = "euclidean", clustering_distance_cols = "euclidean", clustering_method = "ward.D2",
	...
	filename = paste("/scratch/iwt/ehsab/Gene_duplication/Results/SA/Compare_SA__permut_thresold/development/GO/GSEA/SSD_GSEA_PollenRootLeafGO",Type,".pdf", sep = ""))
