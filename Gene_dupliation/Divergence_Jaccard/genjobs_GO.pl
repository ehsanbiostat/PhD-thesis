...
if ($line < 250) {
  ...
  my $rfile="GO".$i.".r";	
  my $jobfile="job".$i."_GO.sh";
  open(OUT,">$jobfile");
  print OUT "module load R\n";
  print OUT "R --vanilla < $rfile > out_GO$i.txt\n";
  close OUT;
  `qsub -S /bin/bash -l hostname="cyclone1|blizzard2|blizzard1|tanith|crunch1|cyclone2|cyclone3|cyclone4|cyclone5|cyclone6|cyclone7|cyclone8|hurricane1|hurricane2" -l h_vmem=0.7G $jobfile`;
