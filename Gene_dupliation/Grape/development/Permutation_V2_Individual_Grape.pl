...
load(\"/scratch/iwt/ehsab/Gene_duplication/Results/SA/Local/Grape/data.norm.RData\")
	...
	thresh_gene_score = apply(gene_scores_matrix, 1, quantile,probs = c(0.99, 0.95, 0.90, 0.85))
	thresh_cond_score = apply(cond_scores_matrix, 1, quantile,probs = c(0.99, 0.95, 0.90, 0.85))

	saveRDS(t(thresh_gene_score), file = paste(\"/scratch/iwt/ehsab/Gene_duplication/Results/SA/Grape/Permutation/Gene_score/V2/Threshold/T\",j,\".rds\", sep = \"\"))
    saveRDS(t(thresh_cond_score), file = paste(\"/scratch/iwt/ehsab/Gene_duplication/Results/SA/Grape/Permutation/Condition_score/V2/Threshold/T\",j,\".rds\", sep = \"\"))
