...
length(which(AA > 1))
TT = foreach (m = 1:1000, .combine = rbind) %dopar% { 
	print(m)
...
TT.para = TT.filter[, c(6, 14, 30)]
TT.para = TT.para[!duplicated(TT.para), ]
write.table(TT.para, file = "/scratch/iwt/ehsab/Gene_duplication/Results/SA/Grape/Validation/Compendium_wise/development/Para.list.txt", row.names = F, quote = F, sep = "\t")

TT.para.no = TT[, c(6, 13,)]
TT.para.no = TT.para.no[!duplicated(TT.para.no), ]
write.table(TT.para.no, file = "/scratch/iwt/ehsab/Gene_duplication/Results/SA/Grape/Validation/Compendium_wise/development/Para.list.txt", row.names = F, quote = F, sep = "\t")
...
# Anchorpoints modules

TT.anch = TT[TT[, 6]%in% paralogous$Ref & TT[, 14] %in% paralogous$Anch ,]
TT.anch.filter = TT.anch[which(TT.anch$Paralogous.cluster > 2), ]
TT.anch.para = TT.anch.filter[, c(6, 14, 30)]
TT.anch.para = TT.anch.para[!duplicated(TT.anch.para), ]
nrow(paralogous)
TT.anch = foreach(i = 1:20, .combine = rbind) %dopar% {
	print(i)
	TT[TT[, 6]%in% paralogous$Ref[i] & TT[, 14] %in% paralogous$Anch[i] ,]
}

write.table(TT.anch.para, file = "/scratch/iwt/ehsab/Gene_duplication/Results/SA/Grape/Validation/Compendium_wise/development/Anch.list.txt", row.names = F, quote = F, sep = "\t")
