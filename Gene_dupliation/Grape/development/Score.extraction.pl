...
# TT = foreach (m = 1:length(which(AA > 1)), .combine = rbind) %dopar% { 
	...
	# related scores
	TT = cbind(graph.line[seq.int(1, nrow(graph.line), 2), ], graph.line[seq.int(2, nrow(graph.line), 2), ], data.frame(INTER.Tot1[m]), data.frame(CORR), Arab.scores)
# }
	save(TT, file = paste(\"/scratch/iwt/ehsab/Gene_duplication/Results/SA/Grape/Compendium_wise/development/Score/Score\",m,\", Combined_Score.RData\", sep = \"\")
	"
		;
		close OUT;
	}
