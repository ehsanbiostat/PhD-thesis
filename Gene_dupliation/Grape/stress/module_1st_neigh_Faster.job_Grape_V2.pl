...
for(my $i = 1; $i < 7179; $i++){
     ...
     my $jobfile = "Fir".$i.".modul.fast.grape.r";
     open(OUT,">$jobfile");
     print OUT "
...
# registerDoMC(14)
	...
	memb.comun.grape = data.frame(\"PLAZA.ID\" = gene.names[neighborhood(g.grape, 1, i, mode = c(\"out\"))[[1]],])
	memb.comun.grape.para = merge(memb.comun.grape, orthologous, by = \"PLAZA.ID\")
	memb.comun.grape.para.size = length(unique(memb.comun.grape.para\$PLAZA.ID))
	...
	for (j in 1:length(V(g))){
			...
			Score[j, \"orthologous.cluster\"] = length(unique(merge(memb.comun.Arab, memb.comun.grape.para, by = \"gene_id.y\")\$PLAZA.ID))
	...
	write.table(Score, file = paste(\"/scratch/iwt/ehsab/Gene_duplication/Results/SA/Grape/Compendium_wise/\",NAME[k],\"/Score99/Scores_V2/Score".$i.".txt\", sep = \"\"), row.names = F, quote = F, sep = \"\\t\")
