...
for(my $j = 1; $j < 15; $j++){
	...
	my $jobfile = "SA".$j."_Permut.SD_V2.r";
     open(OUT,">$jobfile");
     print OUT "
...
foreach (j = 1:nrow(data)) %dopar% {
	...
	sampled.Ec = apply(data.norm\$Ec, 1, sample)
	sampled.Ec = t(sampled.Ec)
	...
	sampled.Er = apply(data.norm\$Er, 1, sample)
	sampled.Er = t(sampled.Er)
	...
	save(gene_score_module, file = paste(\"/ngsprojects/iwt/ehsab/Gene_duplication/Results/SA/Gene_score/Compendium_wise/\",as.name(NAME[".$j."]),\"/Permutation/V2/G\",j,\".RData\", sep = \"\"))
	save(cond_score_module, file = paste(\"/ngsprojects/iwt/ehsab/Gene_duplication/Results/SA/Condition_score/Compendium_wise/\",as.name(NAME[".$j."]),\"/Permutation/V2/C\",j,\".RData\", sep = \"\"))
